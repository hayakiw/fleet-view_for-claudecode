...
- 部署ごとに役割がまとまっています（組織図では左から経営→営業→開発→デザインの順に表示）:
  - **経営**: 🎯マネージャー（目標を具体的な指示に分解し、各役割へ振り分ける調整役。
    自分では実装・調査・執筆はしない）
...
- 各役割には専用のスキル（`.claude/skills/`）も紐づいています:
  ...
  `lp-design`（LP用）、`system-ui-design`（システム用）、`goal-decomposition`（マネージャー用）
- **対象プロジェクトは事前に保存しておけます**: 「＋ 追加」でフォルダを選び、名前を
  付けて保存すると、以降は入力欄でその名前を**選ぶだけ**になります（毎回パスを
  入力・参照する必要はありません）。不要になったら「🗑」で削除できます
- 使い方: 「組織図」タブの入力欄には**「📝 指示」**と**「🎯 目標」**の2モードがあります:
  - **指示**（既定）: 対象プロジェクトを選び、やってほしいことを自由文で書いて
    送信するだけです。指示の内容（キーワード）から、どの役割が担当するか自動で
    振り分けられます（例:「レビューして」→レビュアー、「設計して」→アーキテクト、
    「調査して」→リサーチャー、「スライドにまとめて」→プレゼンター、
    「議事録を作って」→議事録、「脆弱性をチェックして」→レビュアー、
    「便利になる機能を提案して」→機能提案、「LPを作って」→LP、
    「システム画面をデザインして」→システム、それ以外→エンジニア）。
    1つの役割で完結する単発タスク向けです
  - **目標**: 複数の役割にまたがりうる、より上位の目標（例:「もっと使いやすくして」
    「セキュリティを高めたい」）を書いて送信すると、🎯マネージャーに渡されます。
    マネージャーは`GET /api/roles`で現在の役割一覧を確認し、目標を具体的なタスクに
    分解した上で、それぞれ`POST /api/roles/<ロールID>/run`（自動振り分けを経由しない、
    ロール指定の直接起動API）で個別に振り分けます
