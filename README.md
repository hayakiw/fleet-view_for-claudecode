...
`TASKS.md` に改善候補の一覧、`AGENT_INSTRUCTIONS.md` に自動開発エージェント向けの
作業ルール（1件だけ選ぶ／動作確認の仕方／コミットの仕方／pushはしない、等）を
まとめてあります。

`TASKS.md` はファイルを直接編集しなくても、ダッシュボードの「タスク」タブから
追加・完了チェック・削除ができます。

**実行方法は手動のみ**（Windowsタスクスケジューラやクラウド上のRoutineは使わない方針）。
...
- 1回の起動で1件だけ実装→動作確認→ローカルgitコミットまで行い、`TASKS.md` の該当項目に
  チェックを入れます。pushは行いません（リモート自体を設定していません）。
- 定期的に回したくなった場合も、まずは手動でこのプロンプトを都度実行する運用とし、
  Claude Code以外の外部スケジューラ（タスクスケジューラ／クラウドRoutine）は使わない方針です。

## 組織図（役割ベースのエージェント）

「タスク」タブがFleetView自身の開発専用なのに対し、「組織図」タブは**どのプロジェクトでも**
使える汎用機能です。
