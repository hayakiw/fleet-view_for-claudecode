...
「システム開発も自動化したい」という要望に対しては、次のような方向性を想定しています。
必要になったタイミングで着手します:

1. **バックログ駆動の自動開発** — 本リポジトリに `TASKS.md`（改善候補一覧）を置き、
   `schedule` スキルや cron で Claude Code エージェントを定期起動して、
   バックログを1件ずつ消化・コミットさせる。
2. **セルフモニタリング** — このダッシュボード自体の開発を行う Claude Code
   セッションも、当然このダッシュボードに表示される（自己観測ループ）。
3. **チーム利用への拡張** — 将来的に複数人・複数マシンで使いたくなった場合は、
   サーバーをクラウド（Cloudflare Workers + Durable Objects 等）に載せ替える形で
   拡張できるよう、REST/WebSocket の境界を意識して設計してあります。

これらは現時点では未実装です。必要であれば具体的な要件を教えてください。
